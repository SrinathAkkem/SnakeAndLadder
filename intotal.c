...
int maingame()
{
    int dicer=0,sp=0,i,j,flagx=0,flagy=0;
    char re;
    system("cls");                          
    layout();                          
    position();
    ...
    for (i=0;i<10;i++)
    {
        ...
        else if (sp==22) printf ("Result\t\t Proceed\n");
        else if (sp%2==1)
        {
            playfilesync("sounds/sp.wav");
            printf ("Result\t\t ~sp\n");
            playfile("sounds/MAIN02.wav");
        }
        else
...
void layout()
{
    int box[6]={201,205,187,186,200,188};                            
    FILE *p;
    HANDLE hStdout;                                                    
    int i,j,n;
